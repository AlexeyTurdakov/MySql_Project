...
-- СОЗДАДИМ СНАЧАЛА БАЗУ ДАННЫХ И ЗАПОЛНИМ ЕЕ.

CREATE DATABASE lesson7_shop;
...
и таблица городов cities (label, name). Поля from, 
to и label содержат английские названия городов, 
поле name — русское. Выведите список рейсов flights с русскими названиями городов.*/

-- Создадим таблицы и заполним их
...
CREATE TABLE cities (
	id SERIAL PRIMARY KEY,
    labele VARCHAR(255) COMMENT 'Код города',
    name VARCHAR(255) COMMENT 'Название города'
    ) COMMENT = 'Словарь городов';
    
-- Воспользуемся join соединениями
select * from cities;
SELECT 
	f.id,
